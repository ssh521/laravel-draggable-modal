...
@php
$modalId = $attributes->get('id', 'draggable-modal-multi-' . uniqid());
...
" class="fixed inset-0 pointer-events-none" {{ $attributes->merge(['class' => '']) }}
    id="{{ $modalId }}"
    ...
    <div x-ref="modal" x-show="isOpen" @keydown.escape.window="close" @mousedown.stop="bringToFront" :style="style"
        class="bg-white dark:bg-gray-800 rounded-lg shadow-2xl dark:shadow-gray-900/50 absolute p-0 min-w-[300px] pointer-events-auto border border-gray-200 dark:border-gray-700">
